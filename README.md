...
在无限画布上创建文本/图片节点，选中若干节点交给「Agent 面板」，选择模型（自定义 API 或 ComfyUI 工作流）生成新图片，结果自动落地为新节点并与输入节点连线。
...
| 右键菜单新增节点 | 已实现 |
| 节点右侧 `+` 继续创建并自动连线 | 每个节点 hover 时右侧出现 `+`，点击后新建节点并连线 |
...
暂不包含：视频/音频节点、素材库（Asset Library）、模板、多人协作、团队/多工作空间——按你的需求这些先不做，后续可以按同样的模式扩展。
...
- **新建节点**：进入项目画布后，左上角 `+`、双击画布空白处、右键画布空白处，三种方式都可以新建文本/图片节点。
- **从已有节点继续**：把鼠标移到节点上，右侧会出现 `+`，点击后在右侧创建新节点并自动连一条线，对应 TapNow 文档里 "Continue from one node"。
...
- 目前只有文本/图片两种节点类型，未实现视频、音频、素材库、模板等 TapNow 的其它能力。
