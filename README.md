...
| 工作流库 / Skill 库管理面板 | 右上角「工作流库」「Skill 库」入口，集中查看/新建/编辑/删除所有已保存的 ComfyUI 工作流和 Skill 模板，不用逐个打开节点才能看到都存了什么 |
...
- **ComfyUI 工作流节点**：从新建节点菜单 PART2 里选择「ComfyUI 工作流」创建节点，点击节点上的「配置」填写服务地址、粘贴 workflow JSON，并手动添加任意数量的输入/输出槎位（每个槎位对应节点两侧的一个连接点）。配置弹窗里还可以「从库加载」已保存的工作流，或把当前配置「保存到工作流库」。把上游文本/图片节点连到对应输入连接点后，点击「运行工作流」提交执行，产出的图片会显示在对应输出槎位的缩略图里。
...
- **工作流库 / Skill 库管理面板**：首页顶部和项目画布右上角都有「工作流库」「Skill 库」入口，打开后左侧是所有已保存条目的列表（工作流显示输入输出数量，Skill 显示更新时间），右侧是选中条目的完整编辑区，支持新建/编辑/删除，是查看"仓库里都存了什么"的统一入口，不需要在某个节点里才能看到。
...
配置好的工作流可以保存进「工作流库」（`server/data/comfy-workflows.json`，通过 `/api/comfy-workflows` 增删改查），下次新建 ComfyUI 工作流节点时直接从库加载，不用重新粘贴 JSON、重新配置槎位。右上角「工作流库」入口可以集中查看/管理所有已保存的工作流。

## Skill 库

Skill 是一段预设好的 Markdown 提示词模板（比如"文生图提示词模板""图片反推文字""图转视频提示词模板"），存放在 `server/data/skills.json`，通过 `/api/skills` 提供增删改查。Skill 节点可以从库里选择已有条目加载，也可以直接编辑后选择「仅用于此节点」（不回写库）或「保存到库」（同步更新库条目，供其它项目/节点复用）。右上角「Skill 库」入口可以集中查看/新建/编辑/删除所有已保存的 Skill。
